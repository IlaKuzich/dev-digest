...
- **Steps:**
  ...
  2. **helpers.ts** (pure, no I/O):
     ...
     - `parseLinkedIssueRef(body: string | null): number | null` — detect a GitHub issue/PR reference: both `#N` (regex `(?:closes|fixes|resolves)?\s*#(\d+)`, mirror `octokit.ts:128`) AND a full `github.com/<owner>/<repo>/(issues|pull)/(\d+)` URL. Returns the number or null.
     - `extractRepoDocRefs(body: string | null): string[]` — pure. Find links/paths that point to a **plan/spec file inside this repo**: a GitHub `github.com/<owner>/<repo>/blob/<ref>/<path>` URL (extract `<path>`) OR a repo-relative path token (e.g. `docs/plans/foo.md`, `SPEC.md`, `docs/specs/*.md`). Return normalized repo-relative paths. No fs here.
     - `safeRepoPath(root: string, candidate: string): string | null` — pure path-traversal guard using `path.resolve`/`path.relative`: return the absolute path only if it stays within `root` (reject `..`-escapes, absolute inputs, anything resolving outside). Null otherwise. (See spec §"Plan/spec reference resolution".)
     - `estimateTokens(text: string): number` — `Math.ceil(text.length / 4)` (chars/4 heuristic; spec §"Token-saving core" allows this fallback).
     - `buildIntentMessages({ title, body, issue, planSpec, headers }): ChatMessage[]` — `planSpec?: string` is the resolved inline/linked plan-or-spec content (the **primary** motivation when present). System prompt instructs the model to infer intent from available evidence in priority order (**plan/spec → linked issue → body → title → file/hunk headers**) and to always populate `in_scope`/`out_of_scope` from the changed files even with no prose motivation (spec §"Classifier input & graceful degradation"). Wrap every untrusted signal (body, issue, planSpec, headers) plainly; the model returns the `Intent` schema.
  ...
  4. **service.ts** — `class IntentService { constructor(private container: Container) { this.repo = new IntentRepository(container.db); } }`:
     ...
     - `derive(workspaceId, prId, logger?)`:
       ...
       c2. Plan/spec resolution (**non-fatal**, bounded — spec §"Plan/spec reference resolution"): `planSpec` is built by concatenating, in order, (i) inline plan/spec text already in `pull.body` (the body is passed anyway; no extra work — the prompt weights it), and (ii) the content of any **in-repo** doc referenced by link. For each path from `extractRepoDocRefs(pull.body)`: resolve it with `safeRepoPath(cloneRoot, path)`; if valid, read the file from the **local clone** (reuse the same clone-path access the conventions service uses — mirror `conventions/service.ts` file reads; do NOT re-clone), cap the read to the first ~16 KB, and only accept text/doc files (`.md`/`.txt`/no-ext). Wrap each in try/catch — an unresolvable path, a guard rejection, a missing file, or a read error degrades to skipping that ref, never throws. Do NOT fetch external (non-GitHub, non-repo) URLs.
...
- **Out of scope:** Do NOT touch `reviews/run-executor.ts` or the reviews repository (that is T4). Do NOT persist the linked issue to the DB (fetch live). Do NOT add a migration (`pr_intent` exists). Do NOT touch `pr_brief`, Blast Radius, or Smart Diff. Do NOT read `process.env`. Do NOT fetch arbitrary external URLs — only in-repo files (from the local clone, path-traversal-guarded) and GitHub issue/PR via octokit.
...
## Execution notes
- **Test authoring is deferred by user decision (2026-07-15).** This implementation pass ships the feature code only; the `*.test.ts` / `*.it.test.ts` files enumerated in each task's Steps are written in a later session. Implementers still drive each task to a green **`typecheck`** as the verification gate. Plan-verifier should treat missing test files as intentionally deferred, not as unmet plan items.
- **Task grouping for this pass:** T3 and T4 are executed by a **single backend implementer** (shared server/IntentService context; T4 hard-depends on T3 so they cannot parallelize anyway). T1, T2, T5 run as separate parallel implementers. T1 (the reviewer-core seam) completes well before the backend agent reaches its T4 phase.
