...
import { useUpdateAgentContextPaths } from "@/lib/hooks/context-files";
import { useActiveRepo } from "@/lib/contexts";
import { ContextDocAttachList } from "@/components/context/ContextDocAttachList";

/**
 * ContextTab — Agent editor tab for managing attached project-context docs.
 *
 * Uses the active repo from context (same repo that populates the sidebar).
 * When no repo is active, shows an informational empty state.
 */
export function ContextTab({ agent }: { agent: Agent }) {
  ...
  const updatePaths = useUpdateAgentContextPaths();
  ...
  function handleUpdate(newPaths: string[]) {
    updatePaths.mutate({ id: agent.id, paths: newPaths });
  }
      ...
      >
        {docs.length === 0 ? (
          ...
          <ContextDocAttachList
            allDocs={docs}
            attachedPaths={agent.context_doc_paths ?? []}
